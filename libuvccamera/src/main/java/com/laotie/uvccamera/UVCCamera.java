...
import com.laotie.uvccamera.USBMonitor.UsbControlBlock;
...
//            System.loadLibrary("uvc");
            System.loadLibrary("LaoTie_UvcCamera");
            isLoaded = true;
        }
    }
    ...
    public synchronized void open(final USBMonitor.UsbControlBlock ctrlBlock) {
        int result;
        try {
            mCtrlBlock = ctrlBlock.clone(); // 防止频繁打开关闭UVCCamera，导致应用crash，出现 Too many open files.
            Log.d(TAG,"usb fs name:"+getUSBFSName(mCtrlBlock));
...
//            mSupportedSize = nativeGetSupportedSize(100);
//            mSupportedSize = "720";
        }
    ...
    public synchronized void close() {
        Log.e(TAG,"close");
        stopPreview();
        Log.e(TAG,"after stopPreview  mNativePtr:"+mNativePtr);
        if (mNativePtr != 0) {
            nativeRelease(mNativePtr);
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
        }
        Log.d(TAG,"after nativeRelease");
        if (mCtrlBlock != null) {
            mCtrlBlock.close();
            mCtrlBlock = null;
        }
    ...
    public synchronized void newThreadStopPreview(){
        Observable.create((ObservableOnSubscribe<String>) emitter -> {
            stopPreview();
            emitter.onNext("complete");
        }).subscribeOn(Schedulers.single()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(s -> {
                    Log.e("UVCCamera","complete StopPreview");
                });
    }
...
//    	setFrameCallback(null, 0, false);
    ...
    @SuppressLint("CheckResult")
    public synchronized void newThreadDestroy(DestroyCallback destroyCallback) {
        Observable.create((ObservableOnSubscribe<String>) emitter -> {
            destroy();
            emitter.onNext("complete");
        }).subscribeOn(Schedulers.single()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(s -> {
                    if (destroyCallback != null) {
                        destroyCallback.finishDestroy();
                    }
                    Log.e("UVCCamera","complete destroy");
                });
    }
...
// 			  Log.d(TAG, "setZoom:zoom=" + zoom + " ,value=" + z);
    ...
    private static final native int nativeSetPreviewSize(final long id_camera, final int width, final int height, final int min_fps, final int max_fps, final int mode,
                                                         final float bandwidth);

    private static final native String nativeGetSupportedSize(long id_camera);
//    private static final native String nativeGetSupportedSizeTest(long id_camera);
