...
import com.laotie.uvccamera.util.BuildCheck;
import com.laotie.uvccamera.util.HandlerThreadHandler;
...
public final class USBMonitor {
    ...
    private final Object lockDestroyed = new Object();
    ...
    public USBMonitor(final Context context, final OnDeviceConnectListener listener) {
       ...
       /* UsbControlBlock usbControlBlock = GlobalVariables.getInstance().getUsbControlBlock();
        if (usbControlBlock != null) {
            UsbDevice usbDevice = usbControlBlock.mWeakDevice.get();
            if (usbDevice != null) {
                if (!mCtrlBlocks.contains(usbDevice)) {
                    mCtrlBlocks.put(usbDevice, usbControlBlock);
                }
            }
        }*/
    ...
    private final void processConnect(final UsbDevice device) {
        ...
        mAsyncHandler.post(new Runnable() {
            ...
            public void run() {
               ...
               Log.d(TAG,"ctrlBlock:" + ctrlBlock + ",  processConnect:device=" + device);
                if (ctrlBlock == null) {
...
//                GlobalVariables.getInstance().setUsbControlBlock(ctrlBlock);
                if (mOnDeviceConnectListener != null) {
...
//		Log.v(TAG, "getDeviceKeyName:" + sb.toString());
        return sb.toString();
    }
...
//			}
        ...
        private UsbControlBlock(final UsbControlBlock src) throws IllegalStateException {
            ...
            //Add to FIXME USBMonitor.mCtrlBlocks (Because it is HashMap now, it will be replaced if it is added, so no, hang List on List or HashMap?)
        }
        ...
        public synchronized void close() {
           Log.i(TAG,"UsbControlBlock#close 's mConnection:" + mConnection);
            ...
            if (mConnection != null) {
               ...
               Log.d(TAG,"monitor:" + monitor);
                ...
                if (monitor != null) {
                    ...
                    UsbControlBlock usbControlBlock = monitor.mCtrlBlocks.remove(getDevice());
                    if (usbControlBlock != null) { //note 如果不关闭的话有可能导致too many open files
