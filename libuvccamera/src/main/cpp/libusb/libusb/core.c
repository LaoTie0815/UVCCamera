...
struct libusb_device *usbi_get_device_by_session_id(struct libusb_context *ctx,
    ...
    usbi_mutex_lock(&ctx->usb_devs_lock);
    {
        list_for_each_entry(dev, &ctx->usb_devs, list, struct libusb_device) if (
                    dev->session_data == session_id) {
...
libusb_device *LIBUSB_CALL libusb_get_device_with_fd(libusb_context *ctx,
    ...
    // 内でusbi_alloc_deviceが呼ばれた時に参照カウンタは1
    //android_generate_device Reference counter is 1 when usbi_alloc_device is called in
...
static void do_close(struct libusb_context *ctx,
    ...
    libusb_lock_events(ctx);  //modify by huangtiebing because reboot M20 will cause deadlock
    {
    ...
    libusb_unlock_events(ctx); //modify by huangtiebing because reboot M20 will cause deadlock
...
void API_EXPORTED libusb_close(libusb_device_handle *dev_handle) {
    ...
    /* take event handling lock */
    LOGE("libusb_lock_events(ctx)");
    libusb_lock_events(ctx);    // XXX crash  //modify by huangtiebing because reboot M20 will cause deadlock
    {
//         read the dummy data
...
int API_EXPORTED libusb_release_interface(libusb_device_handle *dev,
    ...
    LOGD("interface %d", interface_number);
    usbi_dbg("interface %d", interface_number);
    if (UNLIKELY(interface_number >= USB_MAXINTERFACES)) RETURN(LIBUSB_ERROR_INVALID_PARAM, int);
...
int API_EXPORTED libusb_init2(libusb_context **context, const char *usbfs) {
    ...
    usbi_mutex_static_lock(&default_context_lock);
    {
        ...
        usbi_dbg("libusb v%d.%d.%d.%d", libusb_version_internal.major,
