...
int usbi_handle_transfer_completion(struct usbi_transfer *itransfer,
    ...
    LOGD("transfer %p has callback %p", transfer, transfer->callback);
...
int API_EXPORTED libusb_handle_events_timeout_completed(libusb_context *ctx,
    ...
    if (libusb_try_lock_events(ctx) == 0) {
        if (completed == NULL || !*completed) {
            /* we obtained the event lock: do our own event handling */
            LOGV("doing our own event handling");
    ...
    if (!libusb_event_handler_active(ctx)) {
        ...
        libusb_unlock_event_waiters(ctx);
        LOGV("event handler was active but went away, retrying");
        goto retry;
    }

    LOGV("another thread is doing event handling");
...
int API_EXPORTED libusb_handle_events_timeout(libusb_context *ctx, struct timeval *tv) {
    ...
    LOGD("libusb_handle_events_timeout");
...
int API_EXPORTED libusb_handle_events_completed(libusb_context *ctx,
    ...
    LOGV("libusb_handle_events_completed");
    struct timeval tv;
    tv.tv_sec = 60;
    tv.tv_usec = 0;
...
void usbi_handle_disconnect(struct libusb_device_handle *handle) {
    ...
    while (1) {
        ...
        to_cancel = NULL;
        list_for_each_entry(cur, &HANDLE_CTX(handle)->flying_transfers, list, struct usbi_transfer) if (USBI_TRANSFER_TO_LIBUSB_TRANSFER(cur)->dev_handle ==
                                                                                                        handle) {
                to_cancel = cur;
                break;
            }
