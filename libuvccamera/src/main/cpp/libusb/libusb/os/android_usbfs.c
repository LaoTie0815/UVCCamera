...
int android_generate_device(struct libusb_context *ctx, struct libusb_device **dev,
    ...
    *dev = usbi_alloc_device(ctx, session_id);    // この時点で参照カウンタ=1 	Reference counter = 1 at this point
    if (UNLIKELY(!dev)) {
        RETURN(LIBUSB_ERROR_NO_MEM, int);
    }
    ...
    out:
    if (UNLIKELY(r < 0)) {
        libusb_unref_device(*dev);    // ここで参照カウンタが0になって破棄される 	Here the reference counter becomes 0 and is discarded
        *dev = NULL;
    } else {
...
static int handle_iso_completion(struct libusb_device_handle *handle,    // XXX added saki
    ...
    LOGD("handling completion status %d of iso urb %d/%d", urb->status, urb_idx, num_urbs);
    ...
    if (urb_idx == num_urbs) {
        usbi_dbg("last URB in transfer --> complete!");
        LOGD("last URB in transfer --> complete!");
        free_iso_urbs(tpriv);
        usbi_mutex_unlock(&itransfer->lock);
...
static int reap_for_handle(struct libusb_device_handle *handle) {
    ...
    LOGD("transfer type =%d status=%d transferred=%d", transfer->type, urb->status, urb->actual_length);
