...
int UVCPreview::setPreviewDisplay(ANativeWindow *preview_window) {
    ...
    pthread_mutex_lock(&preview_mutex);
    {
        if (mPreviewWindow != preview_window) {
            LOGI("mPreviewWindow != preview_window");
            if (mPreviewWindow) {
                ANativeWindow_release(mPreviewWindow);
            }
...
//filename =  sprintf(string.jpg)
//1.fd = open(filename, mode = O_WRITE_ONLY | O_READ_ONLY);
//write(fd, data, lenth);
//close;


long UVCPreview::getCurrentTime() {
    struct timeval tv;
    gettimeofday(&tv, NULL);
    return tv.tv_sec * 1000 + tv.tv_usec / 1000;
}

void UVCPreview::write_frame(uvc_frame_t *frame) {
    long current_time = getCurrentTime();
    char path[256];
    sprintf(path, "/sdcard/frame/%ld.jpeg", current_time);
    int fd = open(path, O_CREAT | O_RDWR, S_IWUSR);
    int length = write(fd, ((char *) (frame->data)), frame->data_bytes);
    LOGE("length:%d, data_bytes size :%d",length, frame->data_bytes);
    if (fd > 0) {
        close(fd);
    }
    fd = 0;
...
//                    write_frame(frame_mjpeg);

//                    LOGE("frame's width: %d, frame's height:%d",frame_mjpeg->width,frame_mjpeg->height);
//					LOGD("the frame_mjpeg: %x,the frame: %x",frame_mjpeg,frame);
...
//====================================================================================================
            if (iframecallback_fields.onVerifyFrame != NULL) {
