...
#if 0 // When not outputting debug information 1
#ifndef LOG_NDEBUG
...
//			libusb_set_debug(mContext->usb_ctx, LIBUSB_LOG_LEVEL_DEBUG);
        ...
        // カメラ機能フラグをクリア 		Clear the camera function flag
        clearCameraParams();
        fd = dup(fd);
        // 指定したvid,idを持つデバイスを検索, 見つかれば0を返してmDeviceに見つかったデバイスをセットする(既に1回uvc_ref_deviceを呼んである)
        //Search for the device with the specified vid and id, return 0 if found, and set the found device to mDevice (already called uvc_ref_device once)
//		result = uvc_find_device2(mContext, &mDevice, vid, pid, NULL, fd);
        ...
        if (LIKELY(!result)) {
            // カメラのopen処理, 			Camera open processing

            //only for debug
            LOGE("befor test mDeviceHandle");
            if (mDeviceHandle){
                LOGE("mDeviceHandle is not null");
            } else{
                LOGE("mDeviceHandle is null");
            }
            ...
            if (LIKELY(!result)) {
                ...
                //If the reference counter is 0, mDevice is freed by uvc_unref_device, so it is unnecessary. XXX Crash, I want to try to destroy it again because it has already been destroyed.
...
char *UVCCamera::getSupportedSize() {
    ENTER();
    LOGE("reset_on_release_if:%d", mDeviceHandle->reset_on_release_if);
    if (mDeviceHandle) {
        LOGE("getSupportedSize");
        UVCDiags params;
        LOGE("getSupportedSize");
        return params.getSupportedSize(mDeviceHandle);
...
int UVCCamera::setFrameCallback(JNIEnv *env, jobject frame_callback_obj, int pixel_format, bool isNeedFrameCallback) {
    ENTER();
    int result = EXIT_FAILURE;
    if (mPreview) {
        result = mPreview->setFrameCallback(env, frame_callback_obj, pixel_format, isNeedFrameCallback);
    }
    RETURN(result, int);
}
