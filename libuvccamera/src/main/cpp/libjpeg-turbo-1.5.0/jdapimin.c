...
jpeg_read_header(j_decompress_ptr cinfo, boolean require_image) {
    ...
    LOGD("#111_1 after jpeg_consume_input output_width=%d,   retcode:%d", cinfo->output_width, retcode);
...
jpeg_finish_decompress(j_decompress_ptr cinfo) {
    if ((cinfo->global_state == DSTATE_SCANNING ||
        ...
        if (cinfo->output_scanline < cinfo->output_height) {
            LOGE("JERR_TOO_LITTLE_DATA,  output_scanline:%d,   output_height:%d", cinfo->output_scanline, cinfo->output_height);
